...
SEUIL_SCORE_ULTIME  = 70   # Score /100 minimum pour alerte
SEUIL_SCORE_FORT    = 55   # Signal fort (email moins urgent)
...
<style>
  ...
  .container{{max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.1);}}
  .header{{background:linear-gradient(135deg,#0f2540,#1a3a5c);padding:24px;text-align:center;}}
  .logo{{font-family:Georgia,serif;font-size:22px;font-weight:700;color:#f0d080;letter-spacing:2px;}}
  .subtitle{{font-size:11px;color:rgba(255,255,255,.5);text-transform:uppercase;letter-spacing:2px;margin-top:4px;}}
  .section{{padding:16px 20px;}}
  .section-title{{font-size:11px;font-weight:700;color:#666;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;}}
  .signal-card{{background:#f8f7f4;border-radius:6px;padding:14px;margin-bottom:10px;border-left:4px solid #f0d080;}}
  .signal-card.ultime{{border-left-color:#16a34a;background:#f0fdf4;}}
  ...
  .ticker{{font-size:18px;font-weight:700;color:#0f2540;}}
  .name{{font-size:11px;color:#666;}}
  .score-badge{{font-size:20px;font-weight:700;}}
  .score-badge.ultime{{color:#16a34a;}}
  .score-badge.fort{{color:#d97706;}}
  .grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin:10px 0;}}
  .cell{{background:#fff;border-radius:4px;padding:6px;text-align:center;}}
  .cell-label{{font-size:9px;color:#888;text-transform:uppercase;}}
  .cell-value{{font-size:13px;font-weight:700;color:#0f2540;}}
  .signaux{{font-size:11px;line-height:1.8;color:#444;}}
  .btn{{display:block;text-align:center;background:#0f2540;color:#f0d080;text-decoration:none;padding:12px;border-radius:6px;font-weight:700;font-size:13px;margin:16px 20px;}}
  .footer{{background:#f0ede8;padding:12px 20px;font-size:10px;color:#888;text-align:center;}}
...
"""
    ...
    <div class="signal-card ultime">
      ...
      <div class="grid">
        <div class="cell"><div class="cell-label">Cours</div><div class="cell-value">{a['price']}€</div></div>
        <div class="cell"><div class="cell-label">Stop</div><div class="cell-value" style="color:#dc2626">{a['stop']}€</div></div>
        <div class="cell"><div class="cell-label">Objectif</div><div class="cell-value" style="color:#16a34a">{a['o1']}€</div></div>
        <div class="cell"><div class="cell-label">R/R</div><div class="cell-value" style="color:#1d4ed8">{a['rr']}x</div></div>
      </div>
      <div class="signaux">
"""
...
if __name__ == '__main__':
    ...
    alertes_max  = [r for r in resultats if r['score'] >= SEUIL_SCORE_ULTIME]
    ...
    if alertes_max or alertes_fort:
        nb = len(alertes_max)
        if nb > 0:
            subject = f"🚀 VAL.PEA — {nb} Signal(s) Ultime(s) détecté(s) — {datetime.now().strftime('%d/%m')}"
        else:
            subject = f"⚡ VAL.PEA — {len(alertes_fort)} Signal(s) Fort(s) — {datetime.now().strftime('%d/%m')}"
        
        html = build_email(alertes_max, alertes_fort)
