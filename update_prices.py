...
if __name__ == '__main__':
    ...
    if TELEGRAM_TOKEN and TELEGRAM_CHAT_ID:
        ...
        cac_str = (' · CAC ' + ('+' if (macro.get('cac40_ytd') or 0) >= 0 else '') +
                   str(macro.get('cac40_ytd', '?')) + '%') if macro.get('cac40_ytd') is not None else ''
        msg = '<b>VAL.PEA</b> · ' + now_str + '\n'
        msg += str(ok) + ' cours mis a jour' + fng_str + cac_str + '\n'
        bpf_lines = [t + ' ' + str(prices_live[t]) + 'EUR' for t in ['AI','RMS','ASML','TTE','GTT'] if t in prices_live]
        if bpf_lines:
            msg += 'BPF: ' + ' · '.join(bpf_lines)
        send_telegram(TELEGRAM_TOKEN, TELEGRAM_CHAT_ID, msg)
    else:
        print("  Telegram non configure (secrets manquants)")
