...
def get_fear_greed():
    """Fear & Greed Index depuis alternative.me (gratuit)"""
    try:
...
def get_cac40_ytd():
    """Performance YTD du CAC40 depuis yfinance"""
    try:
        import yfinance as yf
        from datetime import datetime
        cac = yf.Ticker('^FCHI')
        ...
        hist = cac.history(start=start_year)
        if len(hist) >= 2:
            price_jan = float(hist['Close'].iloc[0])
            price_now = float(hist['Close'].iloc[-1])
            ytd = round((price_now - price_jan) / price_jan * 100, 1)
...
if __name__ == '__main__':
    ...
    if TELEGRAM_TOKEN and TELEGRAM_CHAT_ID:
        ...
        fng_str = (' · F&G ' + str(macro.get('fng')) + '/100') if macro.get('fng') else ''
        cac_str = (' · CAC40 ' + ('+' if (macro.get('cac40_ytd',0) or 0) >= 0 else '') + str(macro.get('cac40_ytd','?')) + '% YTD') if macro.get('cac40_ytd') is not None else ''
        msg = '<b>VAL.PEA</b> · ' + now_str + '\n'
        msg += str(ok) + ' cours mis à jour' + fng_str + cac_str + '\n'
        # BPF résumé
        bpf_lines = []
        for t in ['AI','RMS','ASML','TTE','GTT']:
            if t in prices_live:
                bpf_lines.append(t + ' ' + str(prices_live[t]) + '€')
        if bpf_lines:
            msg += '\nBPF: ' + ' · '.join(bpf_lines)
