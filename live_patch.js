// ================================================================
// PEA SCREENER PRO — live_patch.js v4
// Prix live multi-proxy + Analyse IA Anthropic
// ================================================================

const YF_BASE = 'https://query1.finance.yahoo.com/v8/finance/chart/';

// Proxies CORS par ordre de priorité (fallback automatique)
const PROXIES = [
...
window.fetchLive = async function(isAuto) {
  ...
  if (isAuto) {
    try {
      if (Date.now() - parseInt(localStorage.getItem(LS)||0) < 15*60*1000) return;
    } catch(e) {}
  }
  ...
  console.log('[live_patch v4] '+ok+' OK, '+fail+' fail, proxy#'+_activeProxyIdx);
};
...
function showKeyPrompt(onSave) {
  ...
  m.innerHTML = '<div style="background:#0f2540;border-radius:14px;padding:28px;max-width:440px;width:92%;border:1px solid rgba(255,255,255,.1);"><div style="font-size:16px;font-weight:700;color:#fff;margin-bottom:6px;">🤖 Clé API Anthropic</div><div style="font-size:12px;color:#8899aa;margin-bottom:18px;">Pour analyser les actions avec IA + recherche web temps réel</div><input id="ant-inp" type="password" placeholder="sk-ant-api03-..." style="width:100%;padding:11px;background:#1a3050;border:1px solid #2a4060;border-radius:8px;color:#fff;font-size:12px;font-family:monospace;box-sizing:border-box;outline:none;" /><div style="font-size:11px;color:#8899aa;margin-top:8px;">Stockée localement · <a href="https://console.anthropic.com" target="_blank" style="color:#7C3AED;">Obtenir une clé</a></div><div style="display:flex;gap:10px;margin-top:18px;"><button id="ant-save" style="flex:1;padding:10px;background:#7C3AED;color:#fff;border:none;border-radius:7px;font-weight:700;cursor:pointer;">Activer l'IA</button><button id="ant-skip" style="padding:10px 16px;background:transparent;color:#8899aa;border:1px solid #2a4060;border-radius:7px;cursor:pointer;">Plus tard</button></div></div>';
  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages:[{role:'user',content:'Analyse rapide de '+name+' ('+ticker+') PEA. '+ctx+'.
...
function injectIAButton(ticker, name, scoreData) {
  ...
  wrap.style.cssText = 'padding:0 0 8px 0;';
...
setTimeout(()=>{ if (typeof window.fetchLive==='function') window.fetchLive(false); },2500);
console.log('[live_patch v4] chargé | clé IA:',!!window._ANT,'| proxies:',PROXIES.length);
