...
def get_stocks(html_content):
    ...
    for m in re.finditer(r"\{ticker:'([^']+)'(.*?)(?=\n\n\{ticker:|\n\n\];)", s_code, re.DOTALL):
        ...
        # Calculs dérivés
        # "En zone" = cours avec décote >= 10% sur valeur intrinsèque DCF médiane
        # C'est le vrai signal indépendant de la recalibration automatique des zones
        s['in_zone'] = (s['price'] > 0 and s['dcfm'] > 0 and s['price'] <= s['dcfm'] * 0.88 and s['score'] in ['A','B'])
        ...
        s['signal'] = 'ULTIME' if (s['qarp'] >= 70 and s['in_zone'] and s['upside'] > 5 and s['score'] in ['A','B']) else \
